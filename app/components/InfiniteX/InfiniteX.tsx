...
export default function Page() {
  return (
    <section className='relative flex h-fit w-screen items-center'>
        ...
        <div className='animate-marquee flex w-max [--duration:30s] hover:[animation-play-state:paused]'>
          {[...items, ...items].map((item, index) => (
            <div key={index} className='h-full px-2.5'>
              <div className='relative h-full w-[28rem] rounded-2xl border border-white/5 bg-white/5 dark:bg-gray-800 px-8 py-6'>
                <div className='pb-4 font-light text-gray-600 dark:text-gray-200'>
                  {item.body}
                </div>
